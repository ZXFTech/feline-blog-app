...
All content containers default to `default` inner padding (`--spacing-panel-inset-default`) unless explicitly specified otherwise. This applies to page parents, sections, and scroll containers as well as panels: leave space between their clipping boundaries and child shadows. Prefer the default density of `NeuPanel`; plain containers use the same padding token. Zero padding or another density must be explicit at the call site. Preserve the container's height budget and give the scrollable child the remaining inner height after padding and gaps.

Keep `NeuSurface` as the visual primitive rather than an implicitly padded content container. When used as a content container, explicitly apply the default padding token or specify an exception at the call site. Use it directly for visual elevation, edge to edge media, progress tracks, icon containers, navigation, dense rows, or a composite control that already owns its spacing. An intentional exception may override `NeuPanel` spacing, but it should remain visible at the call site.
...
Dialog surfaces follow their purpose by default. Display only dialogs use the Album confirmation dialog visual language through `DialogContent variant="display"`, while retaining normal Dialog dismissal and focus behavior. Form dialogs use the Album general dialog visual language through `DialogContent variant="form"`; this is the default variant. Actions that require an explicit confirmation use `AlertDialog`, whose default surface is also the confirmation visual language. A caller may choose another treatment only by specifying it explicitly at the call site.
