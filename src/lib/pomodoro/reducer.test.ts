...
describe('pomodoroReducer', () => {
  ...
  it.each(['short_break' as const, 'long_break' as const])(
    'skips an unstarted %s without creating a record',
    (phase) => {
      const skipped = pomodoroReducer(
        {
          ...initialState,
          phase,
          remainingMs: durationMsFor(phase, defaultSettings),
        },
        { type: 'SKIP', now: 11_000 }
      );

      expect(skipped).toMatchObject({
        phase: 'focus',
        run: 'stopped',
        remainingMs: durationMsFor('focus', defaultSettings),
        activeEventId: null,
        pendingOutcome: null,
      });
    }
  ...
  it('AC-3 chooses a long break after the configured focus count', () => {
    ...
    expect(completed.remainingMs).toBe(durationMsFor('long_break', configured.settings));
  });
  ...
  it('AC-8 blocks a new start until the pending outcome is acknowledged', () => {
    ...
    expect(pomodoroReducer(completed, { type: 'ACK_OUTCOME', eventId }).pendingOutcome).toBeNull();
  });
});
