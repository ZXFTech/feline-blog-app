...
test("covers: AC-8, blocks starting when local storage is unavailable", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("alert").filter({ hasText: "浏览器存储不可用" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "开始专注" })).toBeDisabled();
});

test("covers: AC-8, quarantines corrupt timer data and recovers safely", async ({
  page,
}) => {
  await login(page, primary);
...
test("covers: AC-4, keeps an offline result visible and syncs it after reconnecting", async ({
  ...
  await expect(
    page.getByRole("status").filter({ hasText: "1 条待同步" }),
  ).toBeVisible({ timeout: 8_000 });
  await expect
    .soft(page.getByText("目标 00:02", { exact: true }).first())
    .toBeVisible();
...
test("covers: AC-7, isolates and restores timer state when switching users", async ({
  page,
}) => {
  await login(page, primary);
  await page.getByRole("spinbutton", { name: "专注分钟" }).focus();
  ...
  await page.keyboard.press("Tab");
  await expect(page.getByRole("spinbutton", { name: "专注分钟" })).toHaveValue(
    "7",
  );
  await page.getByRole("button", { name: "开始专注" }).click();
  await expect(page.getByRole("button", { name: "暂停" })).toBeVisible();
  await page.getByRole("button", { name: "暂停" }).click();
  const primaryRemaining = await page
    .getByLabel(/剩余时间/)
  ...
  await expect(
    secondaryMain.getByRole("button", { name: "开始专注" }),
  ).toBeEnabled();
  ...
  await expect(
    restoredMain.getByRole("button", { name: "继续" }),
  ).toBeVisible();
...
test("covers: AC-5, shows a server conflict and adopts the first record", async ({
  ...
  try {
    ...
    await expect(page.getByText("存在冲突", { exact: true })).toHaveCount(0);
    await expect(
