...
5. Add the protected Vercel Cron purge route, cascade-aware bounded cleanup, backlog continuation, database constraint and privilege verification, migration preflight and retry safety. Complete unit, integration and browser coverage including spec 0007 deferred interactions and all failure states. Satisfies **AC-17**, **AC-18**.

## Migration plan

### Strategy

Keep the existing aggregate transaction boundary and move ChecklistItem to one required `detail` field with no `name` field or per-checklist content uniqueness index. Deployment migrations never delete checklist data. The schema migration aborts before any write when legacy checklist rows exist. A separate reset tool may remove checklist data only in explicitly allowlisted local or staging environments and only after the operator names that environment; it is never invoked by deployment. A migration file that has been applied in any shared environment is immutable, so the change lands as a new forward migration there. Only a provably unapplied pending migration may be edited before first application. The client migration still replaces repeated inline forms with one composer plus a card draft list and moves item recovery into its parent detail Dialog.

### Phases

1. Add a preflight that aborts on legacy rows. For an allowlisted disposable environment, run the separate explicit reset tool, then apply the forward schema change from `name + nullable detail` to one required detail; remove uniqueness enforcement and update constraints, search indexes and owner-scoped projections.
