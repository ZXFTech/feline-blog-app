...
function UserList({ state }: { state: AppState }) {
  const activeUsers = useMemoizedSelector(state, (s) =>
...
const ThemeContext = createContext<ThemeContextValue | null>(null);
const ThemeActionsContext = createContext<ThemeActionsContextValue | null>(
  null,
);
...
const [NotificationProvider, useNotifications] =
  createSafeContext<NotificationContextValue>("Notification");
...
interface UseListActions<T> {
  ...
  update: (
    id: T extends Identifiable ? T["id"] : number,
    updates: Partial<T>,
  ) => void;
  clear: () => void;
  set: (items: T[]) => void;
...
function SearchInput({ onSearch }: { onSearch: (query: string) => void }) {
  ...
  return (
    <input type="search" onKeyDown={handleKeyDown} placeholder="Search..." />
  );
}
...
function PriorityPicker() {
  ...
  return (
    <Select items={priorityOptions} value={priority} onChange={setPriority} />
  );
}
```
...
function SortableList<T extends HasId & { sortOrder: number }>({
  ...
  return (
    <ul>
      {sorted.map((item) => (
        <li key={item.id}>
          Item {item.id} (order: {item.sortOrder})
        </li>
      ))}
    </ul>
  );
