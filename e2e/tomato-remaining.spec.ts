...
test('covers: AC-3, restores one expired phase and syncs it once', async ({ page }, testInfo) => {
  ...
  try {
    await page.reload();
    await expect(page.getByText('目标 00:02', { exact: true }).first()).toBeVisible({
      timeout: 12_000,
    });
...
test('covers: AC-8, keeps the in-memory outcome when writing the outbox fails', async ({
  ...
  await expect(page.getByRole('alert').filter({ hasText: '计时结果尚未安全保存' })).toBeVisible();
  await expect(page.getByRole('button', { name: /开始(?:专注|休息)/ })).toBeDisabled();
});
