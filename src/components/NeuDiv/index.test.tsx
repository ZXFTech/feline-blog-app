...
describe("NeuDiv", () => {
  it("renders its children with native div attributes", () => {
    ...
    expect(screen.getByRole("region", { name: "专注摘要" })).toHaveTextContent("今日完成两次专注");
  });
  ...
  it("applies every supported surface and intensity", () => {
    ...
    for (const [surface, intensity, expectedClass] of cases) {
      const { unmount } = render(<NeuDiv surface={surface} intensity={intensity} />);
  ...
  it("keeps caller classes last and separates the raise interaction", () => {
    ...
    expect(neuSurfaceClassNames({ surface: "flat", className: "p-6" })).toMatch(/p-6$/);
  ...
  it("uses the embossed normal surface by default", () => {
    ...
    expect(screen.getByText("默认表面")).toHaveClass("neu-embossed-normal", "p-1");
  });
});
