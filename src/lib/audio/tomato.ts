...
export function playStartSound(volume = 0.5): void {
  try {
    ...
    logger.warning("无法播放声音:", error);
  }
}

export function playResumeSound(volume = 0.5): void {
  try {
    playBeep(600, 100, volume * 0.6);
    // setTimeout(() => playBeep(800, 100, volume * 0.6), 120);
    setTimeout(() => playBeep(1000, 100, volume * 0.6), 120);
  } catch (error) {
    logger.warning("无法播放声音:", error);
  }
}

export function playPauseSound(volume = 0.5): void {
  try {
    playBeep(1000, 100, volume * 0.6);
    // setTimeout(() => playBeep(800, 100, volume * 0.6), 120);
    setTimeout(() => playBeep(600, 100, volume * 0.6), 120);
  } catch (error) {
    logger.warning("无法播放声音:", error);
  }
}

export function playBreakSound(volume = 0.5): void {
  try {
    playBeep(800, 100, volume);
    setTimeout(() => playBeep(800, 100, volume * 0.6), 120);
    setTimeout(() => playBeep(800, 100, volume * 0.6), 240);
  } catch (error) {
    logger.warning("无法播放声音:", error);
  }
}
...
export function playEndSound(volume = 0.5): void {
  try {
    ...
    setTimeout(() => playBeep(800, 150, volume * 0.6), 170);
    setTimeout(() => playBeep(600, 150, volume * 0.6), 340);
  } catch (error) {
    logger.warning("无法播放声音:", error);
  }
}
